...
import { Material, MaterialWithRisk, RiskLevel } from "../types";
import { Search, Eye, Edit2, Cpu, Check, HelpCircle, Filter } from "lucide-react";

// ─── Risk badge ──────────────────────────────────────────────────────────────
const RISK_BADGE_STYLES: Record<RiskLevel, { bg: string; text: string; dot: string }> = {
...
function RiskBadge({ level }: { level: RiskLevel }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full ${s.bg} ${s.text}`}
      aria-label={`Supply risk: ${level}`}
    >
...
const RISK_LEVELS: Array<"All" | RiskLevel> = ["All", "Low", "Medium", "High", "Critical"];
...
export default function MaterialsTable({
  ...
  // Cast to MaterialWithRisk — risk fields are optional so the component works
  // even when the backend hasn't yet enriched the response.
  const enriched = materials as MaterialWithRisk[];
  ...
  const filtered = enriched.filter(m => {
    ...
    const riskMatch = riskFilter === "All" || m.riskLevel === riskFilter;
  ...
  return (
    <div className="bg-white border border-slate-200 shadow-sm rounded-xl overflow-hidden" id="materials-panel">
      ...
      <div className="p-5 border-b border-slate-200 flex flex-col gap-4">
        ...
        {/* Risk-level filter — segmented control */}
        <div className="flex items-center gap-2 flex-wrap">
          <Filter className="w-3.5 h-3.5 text-slate-400 shrink-0" aria-hidden="true" />
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Supply Risk Filter:</span>
          <div className="flex gap-1 flex-wrap" role="group" aria-label="Filter by supply risk level">
            {RISK_LEVELS.map(lvl => {
              const isActive = riskFilter === lvl;
              const s = lvl !== "All" ? RISK_BADGE_STYLES[lvl] : null;
              return (
                <button
                  key={lvl}
                ...
                >
                  ...
                  {lvl !== "All" && (
                    <span className="ml-1 font-mono">
                      ({enriched.filter(m => m.riskLevel === lvl).length})
                    </span>
                  )}
                </button>
              );
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12">
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-4 px-4 max-w-xs">
                        <div className="font-mono text-[11px] font-bold text-indigo-600 flex items-center gap-1.5 flex-wrap">
                          {mat.id}
                          {/* Supply risk badge */}
                          {mat.riskLevel && (
                          ...
                          {mat.isAiMapped && (
                            <span
                              ...
                              title={mat.isAiMapped ? "AI Mapping Estimated via Gemini: " + (mat as unknown as Record<string,string>).mappingExplanation : ""}
                            >
                        ...
                        <div className="text-[10px] text-slate-400 flex items-center gap-2 mt-1 font-medium flex-wrap">
                          <span>{mat.category}</span>
                          <span>•</span>
                          <span>{mat.vendorName} ({mat.vendorCountry})</span>
                          {mat.leadTimeDays != null && (
                            <span className="text-slate-400">• LT: {mat.leadTimeDays}d</span>
                          )}
                        </div>
                      </td>
        ...
        <div className="xl:col-span-4 p-5 bg-slate-50/50 flex flex-col justify-between border-l border-slate-100" id="material-inspect-sidebar">
          {editingId ? (
            ...
            <div className="space-y-5 animate-fade-in">
              {(() => {
                const mat = enriched.find(m => m.id === selectedMaterialId)!;
                  ...
                  <>
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-1 flex-wrap">
                        <h4 className="text-sm font-bold text-slate-800 leading-tight">{mat.name}</h4>
                        {mat.riskLevel && <RiskBadge level={mat.riskLevel} />}
                      </div>
                    </div>
                    ...
                    {mat.riskScore != null && (
                      <div className={`p-3 rounded-lg border text-xs space-y-1.5 ${
                        mat.riskLevel === "Critical" ? "bg-red-50 border-red-200" :
                        mat.riskLevel === "High"     ? "bg-orange-50 border-orange-200" :
                        mat.riskLevel === "Medium"   ? "bg-amber-50 border-amber-200" :
                                                       "bg-emerald-50 border-emerald-200"
                      }`}>
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Supply Risk Analysis</p>
                        <div className="grid grid-cols-3 gap-2 font-mono text-[10px]">
                          <div>
                            <p className="text-slate-400">Risk Score</p>
                            <p className="font-bold text-slate-800">{mat.riskScore}/100</p>
                          </div>
                          <div>
                            <p className="text-slate-400">Lead Time</p>
                            <p className="font-bold text-slate-800">{mat.leadTimeDays}d</p>
                          </div>
                          <div>
                            <p className="text-slate-400">Geo Risk</p>
                            <p className="font-bold text-slate-800">{mat.geopoliticalRiskScore}/100</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-1.5 pt-1">
                          <span className={`w-2 h-2 rounded-full ${mat.isSingleSource ? "bg-orange-500" : "bg-emerald-500"}`} />
                          <span className="text-slate-600 text-[10px]">
                            {mat.isSingleSource ? "Single-source supplier — qualify alternates" : "Multi-source supply chain"}
                          </span>
                        </div>
                      </div>
                    )}
