...
interface SimulationControlsProps {
  ...
  /** Commodity display names indexed by slot: [copper, steel, aluminum, nickel] */
  commodityLabels?: [string, string, string, string];
}
...
export default function SimulationControls({
  ...
  /** Returns the display label for a commodity slot. */
  const getCommodityName = (id: "copper" | "steel" | "aluminum" | "nickel"): string => {
    const labelMap: Record<string, number> = { copper: 0, steel: 1, aluminum: 2, nickel: 3 };
    const idx = labelMap[id] ?? -1;
    if (idx >= 0 && commodityLabels?.[idx]) return commodityLabels[idx];
